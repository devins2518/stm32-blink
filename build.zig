...
pub fn build(b: *Builder) void {
    ...
    const exe = b.addExecutable("blink.elf", "src/main.zig");
    exe.setBuildMode(mode);
    exe.setTarget(buildTarget);
    exe.setBuildMode(mode);

    const vector = b.addObject("vector", "src/vector.zig");
    vector.setTarget(buildTarget);
    vector.setBuildMode(mode);
    exe.addObject(vector);

    exe.setLinkerScriptPath("f303re.ld");

    const bin = b.addInstallRaw(exe, "blink.bin");
